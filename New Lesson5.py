...
clean = lambda t: [
    w for w in (re.sub(r"[^a-z0-9']+", "", x.lower()) for x in t.split())
    if w
]
...
def smart_score(base, q1, q2, strong):
    ...
    opp_pen = 0.12 if any(
        (has_any(q1, [a]) and has_any(q2, [b])) or
        (has_any(q1, [b]) and has_any(q2, [a]))
        for a, b in oppA
    ) else 0
...
def show_flow(q1, q2):
    ...
    print("   Strongest:", ", ".join(shared) if shared else "None")
    ...
    if shared:
        ...
        print("   - Model relied on semantic similarity (not exact words)")
